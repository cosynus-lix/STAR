...
{   
    ...
    double intersection_volume, volume_output, volume_safe;
    float coverage = 1;
    ...
    for (int i = 0; i < output->lower_matrix.row; i++) {
        ...
        float safe_range = output_to_check ->upper_matrix.data[i] - output_to_check->lower_matrix.data[i];
    ...
    if (criterion == 1) {
        ...
        return intersection_volume / volume_safe;
    }
}
...
PartitionList* compute_partitioning(PartitionInput *partition_input)
{
    ...
    coverage1 = compute_coverage(&output, partition_input->output_to_check, partition_input->nnet, 1);
    coverage2 = compute_coverage(&output, partition_input->output_to_check, partition_input->nnet, 2);
    ...
    if (NEED_PRINT){
        ...
        if (coverage1 >= partition_input->safe_treshold) {
            printf("safe:%f\n", coverage1);
        }
        else if (coverage2 <= partition_input->unsafe_treshold) {
            printf("unsafe:%f\n", coverage2);
        }
        else {
            printf("split further: Coverage 1: %f Coverage 2: %f Volume Ratio: %f \n", coverage1, coverage2, volume_ratio(partition_input->input, partition_input->initial_input));
        }
        {
    ...
    else if (volume_ratio(partition_input->input, partition_input->initial_input) >= 0.05) {
        // SPLIT
